...
class Attention(nn.Module):
    ...
    def compute_query_key_value_scores(self,
        ...
        causal_mask = torch.tril(torch.ones(seqlen, seqlen, device=query.device)).view(1, 1, seqlen, seqlen)
        ...
        for _ in range(max_new_tokens):
            ...
            if temperature == 0.0:
                ...
                if top_k is not None:
                    ...
                    top_k_values, _ = torch.topk(logits_work, min(top_k, logits_work.size(-1)), dim=-1)
                    # Create a mask for values that are not in the top-k
                    mask = logits_work < top_k_values[:, -1].unsqueeze(-1)
                    # Set logits not in top-k to -inf so they get 0 probability after softmax
                    logits_work = logits_work.masked_fill(mask, float('-inf'))
                    # ====================== Implement top-k sampling here ======================

                #TODO
                # ====================== Implement temperature sampling here ======================
                # Scale logits by temperature
                logits_work = logits_work / temperature
